...
function search (crit) {
  ...
  return findMatches(data, crit, opt.searchStrategy, opt).sort(opt.sort.bind(context))
}
...
function findMatches (data, crit, strategy, opt) {
  var matches = []
  // first pass: only search titles
  for (var i = 0; i < data.length && matches.length < opt.limit; i++) {
    var match = findMatchesInTitle(data[i], crit, strategy, opt)
    if (match) {
      matches.push(match)
    }
  }
...
function findMatchesInTitle (obj, crit, strategy, opt) {
  if (!isExcluded(obj['title'], opt.exclude) && strategy.matches(obj['title'], crit)) {
    return obj
  }
}
