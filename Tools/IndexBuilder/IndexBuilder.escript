...
IndexListBuilderVisitor._constructor := fn(){
	this.indexList := [];
};
...
ChapterTree.correctPathes := fn(){
	...
	this.folderPath = basePath + "/" + this.chapterNumber + "_" + ending;
	...
	if(this.isFolder){
		...
		if(this.isLeaf()){
			...
			this.markDownPath = this.folderPath + "/" + mdFile;
...
ChapterTree.createNewTutorial := fn(name, markDownFileOnly){
	...
	var basePath = node.folderPath + "/" + chapterNum + "_" + name;
...
IndexBuilder.recurseFolderDFSBuildTree ::= fn(file, parentNode, rootNode, recurseRootFiles){
 	...
 	if(IO.isDir(file)){
 		 ...
 		 foreach(folderContent as var f){
 		 	recurseFolderDFSBuildTree(f, (recurseRootFiles) ? rootNode : node, rootNode, false);
 		 }
 	}
};
...
IndexBuilder.createIndex ::= fn(rootNode){
	if(!rootNode)
		return;
	
	var idxVisitor = new IndexListBuilderVisitor();
	...
	IO.saveTextFile(rootNode.folderPath + "/index.md", indexDoc);
};
