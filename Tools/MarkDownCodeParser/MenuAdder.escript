...
static extractMenu = fn(content){
	...
	foreach(lines as var line){
		...
		if (line.contains("<body>")){
			var parts = processBodyLine(line);
...
static processBodyLine = fn(line){
	var split = line.split("<body>");
...
static addMenu = fn(htmlFile, menu){
	...
	foreach(lines as var line){
		...
		if(addToAfterBody && !addToMenu)
			...
			if(line.contains("<body>")){
				var content = processBodyLine(line);
				if(content){
					if(content[0] != "")
...
static _recurseFiles = fn(rootFolder){
	...
	foreach(htmlFiles as var file){
		var index = IndexBuilder.getIndex(indexTree, true, file);
